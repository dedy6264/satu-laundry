...
func NewUserAccessRepo(repo repositories.Repositories) userAccessPostgresRepository {
...
func (r userAccessPostgresRepository) Create(access entities.UserAccess) error {
	...
	err = r.repo.Db.QueryRow(query, access.Username, hashedPassword, access.Role, access.IsActive, access.ReferenceLevel, access.ReferenceID).
		Scan(&access.ID)
	if err != nil {
		return err
	}
...
func (r userAccessPostgresRepository) FindByUsername(username string) (entities.UserAccess, error) {
	...
	var lastLogin sql.NullTime
	var password string
	err := r.repo.Db.QueryRow(query, username).Scan(
		&access.ID,
		&access.Username,
		&password,
		&access.Role,
...
func (r userAccessPostgresRepository) Update(access entities.UserAccess) error {
	...
	_, err := r.repo.Db.Exec(query, access.Username, access.Role, access.IsActive, access.ReferenceLevel, access.ReferenceID, access.ID)
	return err
}
...
func (r userAccessPostgresRepository) UpdatePassword(id int, password string) error {
	...
	_, err = r.repo.Db.Exec(query, hashedPassword, id)
	return err
}
...
func (r userAccessPostgresRepository) AuthenticateUser(username, password string) (entities.UserAccess, error) {
	...
	// 	return userAccess, nil // No user found with this username
	// }
	...
	if err != nil {
		...
		return userAccess, err // Password doesn't match
	}
