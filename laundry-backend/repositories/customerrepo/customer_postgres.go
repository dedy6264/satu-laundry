...
func NewCustomerRepo(repo repositories.Repositories) customerPostgresRepository {
	return customerPostgresRepository{repo: repo}
}
...
func (r customerPostgresRepository) Create(customer entities.Customer) error {
	...
	customerQuery := `INSERT INTO pelanggan (id_outlet, nama_lengkap, email, nomor_hp, alamat, created_at, updated_at) 
	          VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING id_pelanggan`
	now := time.Now()
	err = tx.QueryRow(customerQuery, customer.OutletID, customer.Name, customer.Email, customer.Phone, customer.Address, now, now).Scan(&customer.ID)
	if err != nil {
		return err
	}
...
func (r customerPostgresRepository) FindByOutletID(outletID int) ([]entities.Customer, error) {
	...
	rows, err := r.repo.Db.Query(query, outletID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (r customerPostgresRepository) FindAllWithPagination(request entities.DTRequest[entities.Customer]) ([]entities.Customer, int, error) {
	...
	if request.Data.ID != 0 {
		baseQuery += ` and id_pelanggan = ` + strconv.Itoa(request.Data.ID)
	}
	if request.Data.OutletID != 0 {
		baseQuery += ` and id_outlet = ` + strconv.Itoa(request.Data.OutletID)
	}
	if request.Data.Name != "" {
		baseQuery += ` and nama_lengkap ILIKE '%` + strings.ToUpper(request.Data.Name) + `%' `
	}
	if request.OrderBy != "" {
		baseQuery += ` ORDER BY ` + request.OrderBy + ` ` + request.SortBy
	} else {
		baseQuery += ` ORDER BY id_pelanggan ASC`
	}
	if request.Length != 0 {
