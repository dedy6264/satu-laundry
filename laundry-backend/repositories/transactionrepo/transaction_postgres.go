...
func NewTransactionRepo(repo repositories.Repositories) transactionPostgresRepository {
...
func (r transactionPostgresRepository) FindDetailsByTransactionID(transactionID int) ([]entities.TransactionDetail, error) {
	...
	rows, err := r.repo.Db.Query(query, transactionID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (r transactionPostgresRepository) UpdateTransactionStatus(id int, status string) error {
	...
	_, err := r.repo.Db.Exec(query, status, id)
	return err
}
